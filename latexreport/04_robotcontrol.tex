...
\begin{itemize}[noitemsep]
	\item \texttt{HOME()} is called when the connection to the gripper is made. that is to insure the gripper starts in the same position. 
	\item \texttt{GRIP()} function works by the gripper keeps closing until a 80N force is reached.
	\item \texttt{RELEASE()} function works by opening the gripper 5 mm.
	\item \texttt{BYE()} funciton closes the connection to the gripper safely.
\end{itemize}
