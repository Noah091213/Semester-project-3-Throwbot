...
To calculate all of these parameters, a standard already known method is used [reference here, [1] Zhang, Zhengyou, A flexible new technique for camera calibration, IEEE Transactions on pattern analysis
and machine intelligence 22 (2000).]. This uses multiple images of a chessboard to find points with known distances between them in the cameras field of view to calculate the intrinsic camera matrix and lens distortion coefficients. However, these parameters do not correct the perspective shift from the camera being at an angle to the table, the homography must be computed as well. Though, as only the table plane is relevant this becomes somewhat relatively simple.
...
\end{align}
Since each pair of corresponding points $(i', j') \leftrightarrow (i, j)$ gives two equations, and there are 8 unknowns, means at least four point pairs are needed to solve the system. This is set up as a matrix equation $\mathbf{Ap} = \mathbf{b}$ [reference here, [1] Zhang, Zhengyou, A flexible new technique for camera calibration, IEEE Transactions on pattern analysis
and machine intelligence 22 (2000).], where $\mathbf{p}$  contains the unknown homography entries. By solving this, we get the specific values for the homography matrix $\mathbf{H}$, which allows us to convert any pixel coordinate in the image to a physical coordinate on the table.
...
When the radius of the circle is known, the Hough transform uses a two-dimensional parameter space corresponding to the possible center coordinates $(a, b)$. An accumulator array of this parameter space is initialized with all values set to zero.
...
Though the implementation in OpenCV uses a slightly more complex version where the radius $r$ can vary. However, all this does is expand the parameter space to three dimensions where the new axis is the different values $r$ can have.

\subsection{Why Hough Transform?}
Other methods like searching for specific patterns within the defined target or looking at the colors of the target were considered. However, since the target and many other objects within the scene are circular means that the Hough transform is both more versatile and more robust, together with it being very easy to implement with OpenCV compared to any specific algorithm hardcoded to this scenario.
...
Another, perhaps more robust, solution here would be to use the circular holes in the table as the points for the homography as they are also known positions. Though this was not implemented.
...
Undistortion and Rectification: The input image is first undistorted using the intrinsic calibration parameters and then rectified using the homography matrix. This results in a top-down view of the table with perspective distortion removed. (This is done before passing it to the function)
...
The threshold value for edge detection and the circle confidence value are provided by the main program (typically 50 and 30, though these values can be tweaked further). The minimum and maximum radius are constrained to 70-80 mm to ensure that only the defined target is detected.
...
The \texttt{Vision::tableToWorld} function transforms these coordinates into the world frame. This consists of a rotation matrix R and a unit conversion from millimeters to meters:
\begin{align}
