...
to calculate $\mathbf{v}_{\text{0}}$ the function $v_0 = \sqrt{\frac{g \cdot \sqrt{x² + y²}}{2 \cos^2(\theta_z) (\sqrt{x² + y²} \tan(\theta_z) - z)}}$ where $\theta_z$ is pitch, is used. \newline
%note:  22.5 deg minimum pitch?
%note:  1000 steps for pitch?
it is best to find the pitch which has the lowest $\mathbf{v}_{\text{0}}$ because that puts the least strain on the robot. to achieve this, it is needed to run through all pitches that make sense and calculate the $\mathbf{v}_{\text{0}}$ for each pitch.  pitches where it is needed to find a path is  $[0^\circ, 90^\circ[$. as finding a path below $0^\circ$ is not always possible and finding path with $90^\circ$ or above is impossible unless the target is below or behind the release point.
