...
\subsection{Accuracy}
The accuracy shown by the final system does not fully meet the expectations defined in section \ref{sec:ProblemDefinition}. The best results achieved by the system, seen in table \ref{tab:test_throw}, managed only a 68\% success rate compared to the desired 90\% consistency. The remaining 32\% of throws in that test, 3 of 8 where boundary hits, meaning they were very close to being within the tolerances. Increasing the consistency likely comes down to tweaking some of the parameters discussed in the earlier sections. 